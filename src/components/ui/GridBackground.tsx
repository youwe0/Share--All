...
export function GridBackground({
  ...
  const colorValue = color === 'accent' ? 'rgba(59, 130, 246, 0.08)' : 'rgba(156, 163, 175, 0.05)';
...
export function HeroGlow({ className }: { className?: string }) {
  return (
    <div className={cn('absolute inset-0 overflow-hidden pointer-events-none', className)}>
      ...
      {/* Secondary glows */}
      <div
        className="absolute top-20 left-1/4 w-[400px] h-[400px] opacity-20"
        style={{
          background: 'radial-gradient(circle at center, rgba(139, 92, 246, 0.4) 0%, transparent 60%)',
        }}
      />
      <div
        className="absolute top-20 right-1/4 w-[400px] h-[400px] opacity-20"
        style={{
          background: 'radial-gradient(circle at center, rgba(6, 182, 212, 0.4) 0%, transparent 60%)',
        }}
      />
    </div>
  );
